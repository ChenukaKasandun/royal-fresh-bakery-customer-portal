...
public interface CustomerDao extends JpaRepository<Customer, Integer> {
    ...
    @Query(value = "SELECT * FROM cckcakesandbakery.customer as c where c.customer_type_id=?1 and c.customer_status_id <>3; ", nativeQuery = true)
    List<Customer> getCustomerByType(Integer customer_type_id);


//    Find customername of logged customer from database
    @Query(value = "SELECT * FROM cckcakesandbakery.customer as c where c.id in ( select u.customer_id from cckcakesandbakery.customer_login_user as u where u.username =?1 );",nativeQuery = true)
    List<Customer> getCustomerNameByLogin(String username);
